...
1. `StripeSecretKey` - Stripe test API key (sk*test*...)
2. `StripePublishableKey` - Stripe test publishable key (pk*test*...)
3. `StripeWebhookSecret` - Stripe webhook signing secret (whsec\_...)
...
```
✅ Resource Group (rg-printanddeploy-prod)
✅ Key Vault (kv-pd-prod-muurllhg) - 6 secrets
✅ Log Analytics (log-printanddeploy-prod)
✅ Cosmos DB (cosmos-printanddeploy-prod)
   - Database: printanddeploy
   - 4 containers ready

   Cost: $0/month (all in free tier)
```
